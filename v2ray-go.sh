...
function data_processing(){
	...
	if [[ ${determine_type} = "0" ]]; then
		...
		if [[ ${determine_type} = "1" ]]; then
			...
			wget -O "/etc/v2ray/config.json" "https://download.v2ray-install.ml/configs/mkcp-dtls.json"
			if [[ $? -eq 0 ]];then
				clear
				echo -e "${ok_font}V2Ray 配置文件下载成功。"
			else
			...
			wget -O "/etc/v2ray/config.json" "https://download.v2ray-install.ml/configs/h2-path.json"
			if [[ $? -eq 0 ]];then
				clear
				echo -e "${ok_font}V2Ray 配置文件下载成功。"
			else
